...
class BotoFactory:
    ...
    Parameters:
        boto3_capability (function):The boto3 capability to be returned,
                                    e.g. client or resource
        session (boto3.Session):    The session to be used to generate the
                                    capability, very useful for using specific
                                    profile defined in CLI
        service_name (boto3.service): Any AWS service available in boto3,
                                    e.g. EC2, IAM, SQS...
        account_id (str):           The AWS account ID where the capability
                                    from which credentials should be valid
        rolename (str):             The target role to be asssumed, must have
                                    trust to session account ID
        region (str):               (Optional) The region where the capability
                                    should be made from
    """
    def get_capability(self, boto3_capability, session, service_name, 
        ...
        if account_id == '':
            account_id = os.environ['DEFAULT_ACCOUNT']
        if rolename == '':
